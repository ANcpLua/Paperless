...
public class DocumentEndpointTests : IntegrationTestBase
{
    ...
    {
        // Arrange
        var pdfContent = await CreateTestPdfAsync("This is a test PDF document");
    ...
    public async Task UploadDocument_DuplicateFilename_ReturnsBusinessError()
    {
        ...
        var pdfContent = await CreateTestPdfAsync("Original document");
        await Client.PostPdfAsync(BaseUrl, pdfContent, "duplicate-test.pdf");

        // Act - Try to upload with same filename
        var newPdfContent = await CreateTestPdfAsync("Duplicate document");
    ...
    public async Task DeleteDocument_BasedOnStatus_ReturnsExpectedResult(string status, bool canDelete)
    {
        ...
        var document = await CreateDocumentWithStatusAsync(status);

        // Act
        var response = await Client.DeleteAsync($"{BaseUrl}/{document.Id}");
    ...
    {
        var pdfContent = await CreateTestPdfAsync($"Content for {filename}");
        return await Client
            .PostPdfAsync(BaseUrl, pdfContent, filename)
            .ExpectAcceptedAsync<CreateDocumentResponse>();
    }

    private static async Task<byte[]> CreateTestPdfAsync(string content)
    {
        // Using CreatePdf.NET to create a simple test PDF
    ...
    private async Task<DocumentDto> CreateDocumentWithStatusAsync(string status)
    {
        // This is a simplified version - in reality you'd need to:
        // 1. Upload a document
        // 2. Potentially process it to get to the desired status
        // For now, just upload and return as-is
        var uploaded = await UploadTestDocumentAsync($"status-test-{status}.pdf");

        // In a real scenario, you might need to wait for processing or 
        // use a test endpoint to set the status
...
public class DocumentEndpointEdgeCaseTests : IntegrationTestBase
{
    ...
    {
        // Arrange - Create minimal valid PDF
        var emptyPdf = await CreateMinimalPdfAsync();
    ...
    [Test]
    [DisplayName("Concurrent uploads with same filename")]
    public async Task UploadDocument_ConcurrentSameFilename_OneSucceeds()
    {
        // Arrange
        var pdf1 = await CreateTestPdfAsync("Content 1");
        var pdf2 = await CreateTestPdfAsync("Content 2");
        const string filename = "concurrent-test.pdf";

        // Act - Upload both concurrently
        var task1 = Client.PostPdfAsync(BaseUrl, pdf1, filename);
        var task2 = Client.PostPdfAsync(BaseUrl, pdf2, filename);

        var responses = await Task.WhenAll(task1, task2);

        // Assert - One should succeed, one should fail
        var statuses = responses.Select(r => r.StatusCode).OrderBy(s => s).ToList();
        await Assert.That(statuses).Contains(HttpStatusCode.Accepted);
        await Assert.That(statuses).Contains(HttpStatusCode.BadRequest);
    }
    ...
    private static async Task<byte[]> CreateMinimalPdfAsync()
    {
        var tempFile = Path.GetTempFileName();
        await Pdf.Create(Dye.White).SaveAsync(tempFile);
